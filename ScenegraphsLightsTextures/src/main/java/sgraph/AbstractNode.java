 ...
 * provides default methods for many of the methods, especially the ones that
 * could throw an exception Child classes that do not want these exceptions
 * throws should override these methods
 *
 * @author Amit Shesh
 */
public abstract class AbstractNode implements INode {
  ...
  /**
   * The parent of this node. Each node except the root has a parent. The root's
   * parent is null
   */
  ...
  /**
   * By default, this method checks only itself. Nodes that have children should
   * override this method and navigate to children to find the one with the
   * correct name
   *
  ...
  /**
   * Sets the scene graph object whose part this node is and then adds itself to
   * the scenegraph (in case the scene graph ever needs to directly access this
   * node)
   *
   ...
   * transformations should override this method
   */
  ...
  /**
   * By default, throws an exception. Any nodes that are capable of storing
   * material should override this method
   *
  ...
  public List<Light> getLightsInView(Stack<Matrix4f> modelview) {
    List<Light> listLights = new ArrayList<Light>();

    for (Light l : this.lights) {
      Light lnew = new Light(l);
      Vector4f pos = lnew.getPosition();
      pos = modelview.peek().transform(pos);
